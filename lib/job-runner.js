...
var WorkflowJobRunner = module.exports = function (opts) {
    ...
    this.error_mode = opts.error_mode || this.job.error_mode || 'sequential';
        ...
        if (err === 'queued') {
            ...
            if (self.job.onerror && util.isArray(self.job.onerror)) {
                ...
                switch (self.error_mode) {
                    case 'rollback':
                        var index = self.job.chain_results.length,
                            count = self.job.onerror.length - index;

                        chain = self.job.onerror;
                        chain.splice(index, count);
                        chain = chain.reverse();
                        break;
                ...
                return self.runChain(chain, 'onerror_results', callback);
            } else {
                return self.onEnd(err, callback);
            }
        }
