...
// schedule - the schedule object. It'll be saved to the backend with the provided
//       properties.
// meta - Any additional information to pass to the backend which is not
//        schedule properties
// callback - f(err, job) callback will be called with error if something
...
Backend.prototype.runSchedules = function (callback) {
    ...
    for (var i = 0, c = this.schedule_jobs.length; i < c; i++) {
        (function(i) {
            ...
            console.log(scheduleJob.execution + ' = ' + scheduleJob.job_uuid);

            if (scheduleJob.execution === 'queued' && now >= Date.parse(scheduleJob.exec_after)) { // Queued schedule job
                ...
                self.getSchedule(scheduleJob.schedule_uuid, function(err, schedule) {
                    ...
                    factory.job(job, function(err, job) {
            ...
            } else if (scheduleJob.execution === 'running' && scheduleJob.job_uuid) {
                count++;
...
Backend.prototype.createScheduleJob = function (schedule, callback) {
    ...
    if (exists) {
        ...
        CronParser.parseDateExpression(schedule.exec_schedule, { date: new Date() }, function(err, interval) {
            if (err) {
                return callback(new e.BackendInternal('Error while parsing execution schedule expression'));
            }
...
Backend.prototype.updateScheduleJob = function (scheduleJob, callback) {
...
Backend.prototype.validateScheduleTarget = function (schedule, callback) {
    var self = this, filtered;
