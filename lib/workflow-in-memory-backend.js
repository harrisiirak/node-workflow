...
//        job properties
// callback - f(err, job)
Backend.prototype.getJobByTarget = function (target, meta, callback) {
    if (typeof (meta) === 'function') {
        callback = meta;
        meta = {};
    }

    for (var uuid in this.jobs) {
        if (this.jobs[uuid].target === target) {
            return callback(null, deepCopy(this.jobs[uuid]));
        }
    }

    return callback(new e.BackendResourceNotFoundError(sprintf(
        'Job with target \'%s\' does not exist', target)));
};
...
Backend.prototype.runSchedules = function (callback) {
    ...
    for (var i = 0, c = self.schedule_jobs.length; i < c; i++) {
      (function (i) {
        ...
        if (scheduleJob.execution === 'queued' &&
          ...
          async.waterfall([
            ...
            function (next) {
              return self.updateScheduleJobProperty(
                scheduleJob.uuid,
                'execution',
                'running',
                null,
            ...
            // Find schedule
            function (next) {
              self.getSchedule(scheduleJob.schedule_uuid, function (err, schedule) {
                if (err) {
                  return next(err);
                }
            ...
            function (schedule, next) {
              var job = {
                execution: 'queued',
                chain_results: [],
                workflow: schedule.workflow_uuid
              };
          ...
          function (err) {
            self.log.error({err: err}, 'Failed to lock schedule job');
...
WorkflowBackend.prototype.getScheduleJob = function (uuid, callback) {
    if (this.schedule_jobs.length > 0) {
        for (var i = 0, c = this.schedule_jobs.length; i < c; i++) {
            if (this.schedule_jobs[i].uuid === uuid) {
                return callback(null, this.schedule_jobs[i]);
            }
        }
    }

    return callback(new e.BackendResourceNotFoundError(sprintf(
        'Schedule job with uuid \'%s\' does not exist', uuid)));
};
...
WorkflowBackend.prototype.getScheduleJobs = function (schedule, callback) {
    if (this.schedule_jobs.length > 0) {
        ...
        for (var i = 0, c = this.schedule_jobs.length; i < c; i++) {
            if (this.schedule_jobs[i].schedule_uuid === schedule.uuid) {
                scheduleJobs.push(this.schedule_jobs[i]);
            }
        }

        return callback(null, scheduleJobs);
    }
...
WorkflowBackend.prototype.deleteScheduleJob = function (scheduleJob, callback) {
    if (this.schedule_jobs.length > 0) {
        for (var i = 0, c = this.schedule_jobs.length; i < c; i++) {
            if (this.schedule_jobs[i].uuid === scheduleJob.uuid) {
                this.schedule_jobs.splice(i, 1);
    ...
    return callback(new e.BackendResourceNotFoundError(sprintf(
        'Schedule job with uuid \'%s\' does not exist', scheduleJob.uuid)));
};
...
// prop - (String) property name
// cb - callback f(err, value)
WorkflowBackend.prototype.getScheduleJobProperty = function (
    uuid,
    prop,
    callback)
{
    if (this.schedule_jobs.length > 0) {
        for (var i = 0, c = this.schedule_jobs.length; i < c; i++) {
            if (this.schedule_jobs[i].schedule_uuid === uuid) {
                return callback(null, this.schedule_jobs[i][prop]);
            }
        }
    }
...
// Update only the given Schedule job property. Intendeed to prevent
// conflicts with two sources updating the same job at the same time,
