...
WorkflowScheduleRunner.prototype.run = function (callback) {
	...
	self.backend.runSchedules(function(err) {
		if (err) {
			self.log.error({err: err}, 'Error finishing pending schedules');
			return callback(err);
		}
		...
		self.backend.nextSchedules(function(err, schedules) {
			if (err) {
				self.log.error({err: err}, 'Error getting next schedules');
				return callback(err);
			}
			...
			for (var i = 0, c = schedules.length; i < c; i++) {
                ...
                self.backend.createScheduleJob(schedules[i], function(err, scheduleJob) {
                    if (err) {
                        self.log.error({err: err}, 'Error creating schedule job');
                        return callback(err);
                    }
