...
var util = require('util'),
    ...
    WorkflowScheduleRunner = require('./schedule-runner'),
...
var WorkflowRunner = module.exports = function (opts) {
    ...
    this.job_scheduler = new WorkflowScheduleRunner({
        runner: this,
    ...
    self.interval = setInterval(function () {
        self.backend.runnerActive(self.identifier, function (err) {
            ...
            self.backend.isRunnerIdle(self.identifier, function (idle) {
                if (idle === false) {
                    async.parallel({
                        run_scheduler: function (cb) {
                            self.job_scheduler.run(function (err) {
                                if (err) {
                                    self.log.error({err: err},
                                        'Error running job scheduler');
                                }
